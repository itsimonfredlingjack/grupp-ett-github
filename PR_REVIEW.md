...
- **Issue**: The application uses hardcoded credentials (`admin`/`admin123`) for administrative authentication.
- **Risk**: Attackers can easily gain administrative access to the system.
- **Action**: Replace hardcoded credentials with a secure database-backed authentication system or environment variables.
...
- **File**: `app.py`
- **Issue**: The Flask application uses a hardcoded secret key (`"dev-secret-key"`).
- **Risk**: Allows attackers to forge session cookies and potentially bypass authentication or protections (CSRF).
- **Action**: Load `SECRET_KEY` from an environment variable in production.

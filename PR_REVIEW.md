...
**Actionable:** Update `MASK_RULES` to support JSON/YAML separators (`[:=]`) and optional quotes. Example: `r'(?i)("?[\w]*(?:token|secret|password|key)[\w]*"?)\s*[:=]\s*"?[\w\-+/=]{8,}"?'` (ensure the key name is captured in group 1).

### Git Argument Injection
**Severity: Low**
The `run_git` function constructs commands using `f"{base_sha}..{head_sha}"`. If these variables contain hyphens (e.g., `-p`), they might be interpreted as flags by `git diff`.
**Actionable:** Sanitize inputs or use `--` to separate revisions from flags, or validate that SHAs are alphanumeric.

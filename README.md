...
5. **Agenten MÅSTE logga varje iteration**
   En agent som bara läser men aldrig skriver = blind = kaos.
   Varje iteration ska agenten skriva in vad den gjorde och hur den verifierade det.

### Iteration Log Mall (copy-paste till CURRENT_TASK.md)

```markdown
## Iteration Log

### Iteration N
**Change:** `src/fil.py` – kort beskrivning (kopplat till ACx)
**Verify:**
- `pytest -q` → PASS (154 passed)
- `ruff check .` → PASS
**Next:** nästa minimala steg (en rad)
```

**Varför?** Det är en minimal, spårbar diff av verkligheten: ändring + bevis + nästa.
Utan detta börjar agenten gissa, och då blir det kaos igen.

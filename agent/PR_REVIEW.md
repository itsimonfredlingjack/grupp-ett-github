...
`requirements.txt` is missing `Flask-WTF` (required for CSRF protection) and `python-dotenv` (used in `jira_client.py` CLI).
**Action:** Add `Flask-WTF>=1.2.0` and `python-dotenv>=1.0.0` to `requirements.txt`.

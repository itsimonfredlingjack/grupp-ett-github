...
The monitoring endpoints in `src/sejfa/monitor/monitor_routes.py` (e.g., `POST /api/monitor/state`, `/reset`, `/task`) are unauthenticated. This allows any network user to inject false events, reset the dashboard state, or update task info without authorization.
...
### 8. Missing Dev Dependency: python-dotenv (Reliability)
The `scripts/preflight.sh` script imports `dotenv` to load environment variables, but `python-dotenv` is missing from `requirements.txt`. This causes the preflight check to fail in environments where it's not installed globally or via `pyproject.toml`.

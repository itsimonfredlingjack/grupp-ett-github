...
The script `scripts/preflight.sh` relies on `python-dotenv` to load environment variables, but this dependency is missing from `requirements.txt`. This will cause the script to fail in environments where `python-dotenv` is not pre-installed.

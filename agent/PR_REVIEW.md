...
### 1. Missing CI Permission (Security)
The `jules_review.yml` workflow requires the `statuses: write` permission to report the review status to the commit. This is missing, causing the workflow to fail silently or report incorrectly.
**Action:** Add `statuses: write` to the `permissions` block in `.github/workflows/jules_review.yml`.
...
### 7. Insecure Secret Key (Security)

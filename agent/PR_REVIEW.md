...
The `MonitorService` relies on unprotected in-memory dictionaries (`nodes`, `event_log`) without thread-safe locking. In a threaded environment (e.g., Flask development server or Gunicorn with threads), concurrent requests could corrupt the state.
**Action:** Add `threading.RLock` to protect access to shared state in `MonitorService`.

### 8. Missing Dependency: python-dotenv (Reliability)
The `scripts/preflight.sh` script relies on `python-dotenv` to load environment variables, but it is missing from `requirements.txt`. This causes the health check script to fail in fresh environments.

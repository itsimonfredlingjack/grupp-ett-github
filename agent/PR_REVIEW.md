...
The `scripts/preflight.sh` script relies on `python-dotenv` to load environment variables, but it is missing from `requirements.txt` (though present in `pyproject.toml`). This causes failures when running scripts in environments installed via `requirements.txt`.

...
The application uses `datetime.utcnow()` in `monitor_service.py` and `monitor_routes.py`, which is deprecated in Python 3.12+.
**Action:** Replace with `datetime.now(datetime.UTC)` or `timezone.utc`.

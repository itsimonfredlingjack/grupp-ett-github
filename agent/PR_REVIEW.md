...
### 8. Deprecated Timestamp Usage (Maintainability)
The codebase uses `datetime.utcnow()` (deprecated in Python 3.12) and `datetime.now()` without timezone info. This can cause timezone-related bugs.
**Action:** Replace with `datetime.now(timezone.utc)` and ensure all timestamps are timezone-aware.
